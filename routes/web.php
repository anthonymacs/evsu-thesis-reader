...
use App\Http\Middleware\EnsureUserIsPending;
...
Route::get('documents/{document}', \App\Livewire\Home\DocumentShowPage::class)
    ->middleware(['auth', EnsureUserIsNotSuspended::class, EnsureUserIsApproved::class])
    ->name('home.documents.show');
...
Route::middleware(['auth', EnsureUserIsPending::class])
    ->get('pending-approval', \App\Livewire\Auth\PendingApproval::class)
    ->name('pending-approval');

Route::middleware(['auth', EnsureUserIsNotSuspended::class, EnsureUserIsApproved::class])
    ->prefix('dashboard')
    ->group(function () {
        ...
        Route::prefix('read-later')->group(function () {
            Route::get('', \App\Livewire\ReadLater\IndexPage::class)->name('read-later.index');
        });
